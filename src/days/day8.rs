...
fn get_scenic_score(height: u8, (x, y): (usize, usize), trees: &Vec<Vec<u8>>) -> usize {
...
pub fn run() {
    ...
    for (i, line) in trees.iter().enumerate() {
        for (j, tree) in line.iter().enumerate() {
            if is_visible(tree.clone(), (j, i), &trees) {
                sum += 1;
            }
            scores.push(get_scenic_score(tree.clone(), (j, i), &trees))
        }
    }
